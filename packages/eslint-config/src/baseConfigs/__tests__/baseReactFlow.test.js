/**
 * @file Unit tests for base React Flow ESLint configuration.
 */
...
describe('Base React Flow ESLint Configuration', () => {
  let config;

  beforeEach(() => {
    jest.clearAllMocks();
    const [, mainConfig] = getBaseReactFlowConfig();
    config = mainConfig;
  });

  describe('getConfig function', () => {
    test('should return an array of configurations', () => {
      const configs = getBaseReactFlowConfig();
    ...
    test('should pass noUnresolvedIgnoreArr to import/no-unresolved rule', () => {
      ...
      const configs = getBaseReactFlowConfig(noUnresolvedIgnoreArr);
      const reactFlowConfig = configs[1];
      expect(reactFlowConfig.rules['import/no-unresolved']).toEqual([
        2,
        {
