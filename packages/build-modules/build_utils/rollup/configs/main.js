...
import getBaseConfig from './main.common.js';
import getMinimizerConfig from './minimizer.js';
import getVisualizerConfig from './visualizer.js';
import getBuildStatsConfig from './buildStats.js';
import { ERR_NO_LIB_ENV_FLAG } from '../../config/logs.js';
...
function getAddons(projectRoot) {
  ...
  const configs = [];
  if (addMinimizer) configs.push(getMinimizerConfig());
  if (addVisualizer)
    configs.push(
      getVisualizerConfig(
...
function getConfig() {
  ...
  const baseConfig = getBaseConfig(projectRoot, process.env.LIB_ENV, []);
